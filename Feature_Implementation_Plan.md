...
### 완료 조건
- [ ] 틀린 문제가 DB에 정상 저장(Upsert)되는지 확인
- [ ] 오답 목록이 정상적으로 조회되는지 확인
- [ ] 복습 후 정답 시 DB에서 삭제되는지 확인
...
### API / DB 변경(선택)
- API : `POST /api/favorites/{questionId}`, `DELETE /api/favorites/{questionId}`, `GET /api/favorites`
- DB : `favorites` 테이블 활용
...
- 목록 조회(페이징) 구현됨
- 미완료:
  - 인증 사용자 연동(`memberId=1L` 하드코딩 제거 필요)
  - 필터링 조건 확장
  - 재학습 세트 생성(10문제)
...
### 통계
- `StatsController`, `StatsQueryService`, `StatsMapper`는 현재 스텁(빈 구현) 상태
...
- 작업:
  1. 오답 제출 시 Upsert 자동 호출
  2. 정답 제출 시 오답노트 자동 삭제

### Step 3. 즐겨찾기 CUD + 목록
- 목표: 사용자 개인 문제 보관 기능 완성
